...
def run(network_file, batch_size, eps, dataset_X, dataset_y, dataset, train, print_intermediate_results, no_sparsity):
	...
	abs_elem = Abs_elem_sparse({'llist' : llist, 'l' : l, 'u' : u}, {'l': 'Float', 'u': 'Float', 'llist': 'bool'}, network, batch_size=batch_size, no_sparsity=no_sparsity)
	flow = Flow(abs_elem, ibp(), network, print_intermediate_results, no_sparsity)
	return flow.flow()
